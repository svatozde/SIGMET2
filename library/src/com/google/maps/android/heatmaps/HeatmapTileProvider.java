...
public class HeatmapTileProvider implements TileProvider {
    ...
    private HeatmapTileProvider(Builder builder) {
        ...
        mKernel = generateKernel(mRadius, mRadius / 15.0);
    ...
    public Tile getTile(int x, int y, int zoom) {
        ...
        int[][] counts = new int[TILE_DIM + mRadius * 2][TILE_DIM + mRadius * 2];
        for (WeightedLatLng w : points) {
            Point p = w.getPoint();
            int bucketX = (int) ((p.x - minX) / bucketWidth);
            int bucketY = (int) ((p.y - minY) / bucketWidth);
            
            intensity[bucketX][bucketY] = ((counts[bucketX][bucketY]*intensity[bucketX][bucketY]) +(w.getIntensity()))/(++counts[bucketX][bucketY]);
        }
